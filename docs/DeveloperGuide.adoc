...
=== [Proposed] Schedule version comparison
==== Proposed Implementation

The proposed schedule version comparison feature utilises `TimeTable` and allows the user to schedule multiple activities within the same time slot. Additionally, the following command is supported:
...
* `schedule 1 st/1300 et/1400 d/1`

Step 4. User schedules the second activity into the same day:

* `schedule 2 st/1300 et/1400 d/1`

Step 5. User executes `optimise budget` command. The activities in that day would be shown in a way such that the activities with the lowest costs are in focus.
