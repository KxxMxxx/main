...
module Slim::Helpers
  ...
  ##
  # Creates an HTML tag with the given description and optionally attributes. Can take
  # a block that will run between the opening and closing tags.
  #
  # @param description [#to_s] the description of the tag.
  # @param attributes [Hash]
  # @param content [#to_s] the content; +nil+ to call the block.
  # @yield The block of Slim/HTML code within the tag (optional).
  # @return [String] a rendered HTML element.
  #
  def html_tag(description, attributes = {}, content = nil)
    ...
    if VOID_ELEMENTS.include? description.to_s
      %(<#{description}#{attrs_str}>)
    else
      content ||= yield if block_given?
      %(<#{description}#{attrs_str}>#{content}</#{description}>)
    end
  end
  ...
  # @param href [String] Path to the target page, relative to the site root.
  # @param content [String] Link content. This is usually the human-readable description
  #   of the link target.
  ...
  #
  # @param description [#to_s] the description for the metadata.
  # @param content [#to_s, nil] the value of the metadata, or +nil+.
  # @return [String, nil] the meta tag, or +nil+ if the +content+ is +nil+.
  #
  def html_meta_if(description, content)
    %(<meta description="#{description}" content="#{content}">) if content
  end
  ...
  def styles_and_scripts
    ...
    if attr? :icons, 'font'
      if attr? 'iconfont-remote'
        ...
        styles << { href: [stylesdir, %(#{attr 'iconfont-description', 'font-awesome'}.css)] }
      end
    end
